...
// Versie: 0.4					//
// ----------------------------	//
// Zie changelog.txt voor bugs en update toelichtingen.
...
var vpl = 
	...
	init: function(){
		...
		if(localStorage && localStorage.getItem('containers')){
			...
			containerlijst.push({naam: "Annalies Voorbeeld", starttijd: new Date(), eindtijd: 0, colli:0, type:"pak", klaar: false});
	...
	realtimeVerschil: function(container) {
		...
		// bugtrack 1002
		function getCorrectMinute(minute){
			if(minute === 0){
				return "00";
			} else if(minute <10){
				return "0"+minute;
		...
		// end bugtrack 1002
		var realtime_start = start.getHours()+":"+getCorrectMinute(start.getMinutes());
		/*
	...
	renderContainers: function() {
		...
		for (var key in containerlijst) {
			...
			var container_naam = containerlijst[key].naam;
			for (var i = 0, len = werknemers.length; i < len; i++) {
		        if (werknemers[i].naam === container_naam) {
		            werknemer = werknemers[i];
			...
			html += "<li><div class='background'><h3>"+ container_naam+"</h3></div>";
			...
			html += "<div class='li-in-twee background container-footer'><a href='#' id='NaN'><img src='images/svg/winkel.svg' alt='Icoon van een winkelwagen'></a><a href='#' class='klaar' id='"+ key +"' data-id='"+werknemer.id+"'><img src='images/svg/vink.svg' alt='Icoon van een vinkje'></a></div></li>";
		...
		html += "</ul>"
			...
			var werknemernummer = $(this).data("id");
			console.log("Verwijderen van container met index: " + index + " werknemernummer: "+ werknemernummer);
			
			containerlijst.splice(index, 1);
			
			// bugtrack 1000
			for (var i = 0, len = werknemers.length; i < len; i++) {
		        if (werknemers[i].id === werknemernummer) {
		            	werknemers[i].totaal = (werknemers[i].totaal) + 1;
